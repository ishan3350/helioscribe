...
const Register = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.howHeard.trim()) {
      newErrors.howHeard = 'Please specify how you heard about us';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      userData.deviceFingerprint = fingerprint;
      userData.recaptchaToken = token; // Token is used here
      ...
      // Token is used directly in the API call, no need to store in state
  ...
  const handleGoogleSignup = async () => {
    // Reset error ref when user explicitly tries to register again
    errorShownRef.current = false;
    
    // Ensure fingerprint is collected before redirecting
    ...
    if (!currentFingerprint) {
      try {
        ...
        console.error('Error collecting browser fingerprint for Google OAuth:', error);
      }
    }
    
    // Redirect to backend Google OAuth registration endpoint with fingerprint
    ...
    const fingerprintParam = currentFingerprint ? `?fingerprint=${encodeURIComponent(currentFingerprint)}` : '';
    window.location.href = `${apiBaseUrl}/api/auth/google/register${fingerprintParam}`;
  };
      ...
      <div className="auth-card">
        ...
        >
          ...
          <div className="form-group">
            ...
            {errors.howHeard && <span className="error-message">{errors.howHeard}</span>}
          </div>
